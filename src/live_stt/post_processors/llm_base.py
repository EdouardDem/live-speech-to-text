"""Shared base classes for prompt-based LLM post-processors."""
...
class LLMProcessor(PostProcessor):
    """Base for processors that render a prompt and call a chat-style API."""

    #: Human-readable provider label (e.g. "Claude", "OpenAI"). Used in logs.
    PROVIDER_LABEL: str = "LLM"

    #: Environment variable consulted when no API key is provided.
    ENV_KEY: str = ""

    #: Label shown in the "API key not configured" error.
    SETTINGS_KEY_LABEL: str = ""

    def __init__(self, cfg: PostProcessorConfig, api_key: str = "") -> None:
        super().__init__(cfg)
        key = api_key or os.getenv(self.ENV_KEY, "")
        if not key:
            raise ValueError(
                f"{self.SETTINGS_KEY_LABEL} API key not configured. "
                f"Set it in Settings → API Keys or via {self.ENV_KEY}."
            )
        self._client = self._build_client(key)

    @abstractmethod
    def _build_client(self, api_key: str):
        """Instantiate and return the provider-specific SDK client."""
    ...
    def run(self, text: str) -> str:
        ...
        log.debug(
            "[%s] → %s (%s): %s…",
            self.cfg.name, self.PROVIDER_LABEL, self.cfg.model, prompt[:80],
        )
...
class LLMForm(Gtk.Grid):
    """Shared GTK form for prompt + model + max_tokens configuration."""
